...
We have so far introduced five rules: Conditional Elimination, \emph{modus tollens}, Disjunction Elimination, Conjunction Elimination, and Conjunction Introduction. There are more rules still to learn, but it is helpful first to pause and draw a distinction between different kinds of rules.
...
The {double turnstile} symbol `$\models$', by contrast, remains unchanged. It stands for semantic entailment, as described in ch.~\ref{ch.SLmodels}.
...
\problempart
For the following, provide proofs using only the basic rules. The proofs will be longer than they would need to be if you used the derived rules.
\begin{earg}
