...
The {\eif}I rule lets us \define{discharge} the assumption we'd been making, ending that vertical line. The horizontal line we draw under line 3 emphasizes that subsequent lines are importantly different: during lines 2 and 3, we were \emph{assuming} that \enot $R$. (Think of the L-shaped pair of lines as defining a box that closes those two lines off.) By the time we get to line 4, we are no longer making that assumption.
...
This is our first example of a natural deduction proof that involves nested assumptions. You can assume something a second new sentence even while a previous assumption is open, as this proof does at line 5. Make sure to close the assumptions in the proper order, closing the newest assumptions first. (Here, the line 5 assumption must be closed, as it is after line 7, before the line 4 one is, at the end of the proof.) The lines that we draw to indicate assumptions help enforce proper assumption resolution --- if you tried to close the line 4 assumption while the line 5 assumption was open, your boxes would cross one another. This is not allowed.
