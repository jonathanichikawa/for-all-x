...
\end{earg}
This is a valid argument in SL. (You can construct a truth table, a tree, or a natural deduction proof to confirm this.) 
...
Our first key notion in QL will be \define{predicates}. A predicate is analogous to an English description--- an expression like `is a dog' or `has black fur'. These descriptions are not a sentence on their own. They are neither true nor false. They are descriptions that apply to some objects, but not to others. In order to be true or false, we need to specify an object: Who or what is it that is said to be a dog, or to have black fur?
...
A quantifier always comes along with an associated variable. It is placed in front of a formula that it binds. Typically that formula will include the same variable. We can translate sentence \ref{q.a} as $\forall x Hx$. Paraphrased partially into English, this means `For all $x$, $x$ is happy.'
