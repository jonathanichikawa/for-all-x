...
\section{Validity and Impossible Premises}

Students are often surprised to learn that this argument is valid:

\begin{earg}
\item Jonathan ran the Boston Marathon.
\item Jonathan has never run a marathon.
\item[\therefore] PHIL 220 is a very popular course at UBC.
\end{earg}

There is no intuitive connections between the premises and the conclusion in this strange argument. Even if you happen to know that Jonathan often teaches PHIL 220, whether an instructor has run the Boston Marathon or any marathon has nothing to do with how popular their courses are likely to be. Even worse, you might think, these two premises seem to be \emph{contradictory}! They couldn't possibly be true, so how could they be part of a valid argument?

But remember, validity has a very specific definition. It's not about whether arguments are good in any intuitive sense; it's about whether they satisfy the definition of validity. An argument is valid if and only if it is impossible for all the premises to be true while the conclusion is false. And in fact, the very point we just observed that seems to make this argument rather stupid --- the fact that its premises couldn't possibly be true together --- is exactly why this argument \emph{is} in fact valid. It's impossible for the premises to be true while the conclusion is false, because it's impossible for the premises to be true \emph{whether or not} the conclusion is false.

For an argument to be \emph{invalid}, there has to be some possible way that the premises could be true while the conclusion is false. Here, that would mean that there's some possible situation where:

\begin{earg}
\item[(1)] Jonathan ran the Boston Marathon.
\item[(2)] Jonathan has never run a marathon.
\item[(3)] PHIL 220 is not a very popular course at UBC.
\end{earg}

There's no way for all three of these to be true, so the argument is valid. All arguments whose premises are not jointly possible are valid, regardless of their conclusion.
