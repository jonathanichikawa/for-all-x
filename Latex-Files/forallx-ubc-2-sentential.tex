...
Sentence \ref{and3} can be paraphrased as `$J$ and $L$.' In order to fully symbolize this sentence, we need another symbol. We will use `\eand'. We translate `$J$ and $L$' as ($J\eand L$). The logical connective `\eand' is called a \define{conjunction}, and $J$ and $L$ are each called \define{conjuncts}.
...
Sentence \ref{or1} is `Either $D$ or $M$.' To fully symbolize this, we introduce a new symbol. The sentence becomes $(D \eor M)$. The `\eor' connective is a \define{disjunction}, and $D$ and $M$ are called \define{disjuncts}.
...
We call the sentence written before the biconditional symbol the \define{left-hand side}. The sentence after it is the \define{right-hand side}. A biconditional tells you that the left-hand side and the right-hand side have the same truth value.
...
\factoidbox{
If a sentence can be paraphrased as `Unless \metaA{}, \metaB{},' then it can be symbolized as $(\enot\metaA{}\eif\metaB{})$, $(\enot\metaB{}\eif\metaA{})$, $(\metaA{}\eor\metaB{})$, or $(\metaB{}\eor\metaA{})$.
}
...
with numerical subscripts, as needed & $A_1, B_1,Z_1,A_2,A_{25},J_{375},\ldots$\\
\hline
...
There are many ways one can translate a given English sentence into SL, which means that a translator has some choices to make. Many of the choices are quite arbitrary. Suppose one wishes to translate this sentence into SL:
